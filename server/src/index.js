...
app.post("/api/analyze", async (req, res) => {
  ...
  try {
    const analysis = await analyzeCodeWithAI(code, baseArray);
    let steps = [];
    ...
    // Priority Routing based on standard algorithms
    ...
    // IF THE AI EXPLICITLY FAILED (API KEY ERROR / QUOTA)
    else if (aiAlgorithm.includes("failed") || analysis.language === "Unknown") {
      steps = [{ array: baseArray, highlight: baseArray.map((_, i) => i), action: "swap", explanation: "AI Analysis Offline: " + (analysis.explanation || "Invalid API Key or Quota Limit Reached.") }];
    }
...
app.post("/api/save-visualization", async (req, res) => {
  const data = req.body;
  const logPath = path.join(__dirname, "..", "..", "correctvisualisation.json");

  const newEntry = {
    id: "viz_" + Date.now(),
    timestamp: new Date().toISOString(),
    page: data.page || "Unknown Page",
    ...
    steps: data.steps || [],
    isCorrect: data.isCorrect !== undefined ? data.isCorrect : false,
    feedback: data.feedback || "Incorrect Visualisation"
  };
  ...
  try {
    ...
    res.json({ success: true, message: "Visualization data saved to correctvisualisation.json successfully." });
  } catch (err) {
    console.error("Failed to write to correctvisualisation.json:", err);
    res.status(500).json({ error: "Failed to write visualization to correctvisualisation.json." });
  }
});

// --- FEEDBACK LOGGING ENDPOINT ---
app.post("/api/feedback", async (req, res) => {
  const { code, array, correct } = req.body;
  ...
  try {
    ...
    res.status(500).json({ error: "Failed to write feedback to log file." });
  }
});
